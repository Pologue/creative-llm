...
class BaiduQianfanLLM(LLMInterface):
    """
    Baidu Qianfan API interface using OpenAI-compatible endpoint.
    
    Supports Qwen models through Baidu's Qianfan platform.
    """
    ...
    def __init__(
        ...
        """
        Initialize Baidu Qianfan LLM interface.
        
        Args:
            model: Model identifier (e.g., "qwen3-14b")
            api_key: Baidu Qianfan API key
            temperature: Sampling temperature
            max_tokens: Maximum tokens in response
            base_url: Baidu Qianfan API base URL
            request_delay: Delay in seconds between requests (default 3.0)
        """
        try:
        ...
        if not api_key:
            import os
            api_key = os.getenv("BAIDU_API_KEY") or os.getenv("QIANFAN_API_KEY")
            if not api_key:
                raise ValueError("Baidu API key must be provided or set as BAIDU_API_KEY/QIANFAN_API_KEY environment variable")
        ...
        # Initialize OpenAI client with Baidu endpoint
        self.client = OpenAI(
            base_url=base_url,
            api_key=api_key
        )
    ...
    def query(self, prompt: str) -> str:
        """Query Baidu Qianfan API."""
        result = self.query_with_usage(prompt)
        return result['response']
    
    def query_with_usage(self, prompt: str) -> Dict[str, Any]:
        """Query Baidu Qianfan API with usage tracking."""
        import time
        ...
        try:
            ...
            traceback.print_exc()
            return {
                'response': f"Error querying Baidu Qianfan: {str(e)}",
                'usage': {'prompt_tokens': 0, 'completion_tokens': 0, 'total_tokens': 0},
                'cost': 0.0
            }
    ...
    def get_name(self) -> str:
        """Get the model name."""
        return f"BaiduQianfan({self.model})"
    
    def get_model_pricing(self) -> Dict[str, float]:
        """Get pricing per 1M tokens for Baidu Qianfan models."""
        # Pricing in dollars per 1M tokens (estimated, adjust based on actual pricing)
        pricing_map = {
            'qwen3-14b': {'input': 0.5, 'output': 1.5},
            'qwen-turbo': {'input': 0.3, 'output': 0.9},
            'qwen-plus': {'input': 0.8, 'output': 2.4},
        }
        return pricing_map.get(self.model, {'input': 0.5, 'output': 1.5})
