...
from modules.llm_interface import LLMInterface, OpenRouterLLM, OpenAILLM, AnthropicLLM, BaiduQianfanLLM
...
class CausalBenchmarkEnhanced:
    ...
    def create_prompt(self, observations: List[Dict], prior_hypotheses: List[CausalGraph]) -> str:
        """Create prompt for LLM - supports both simple and reasoning-based responses."""
        ...
        Response Format:
        You may provide reasoning before your answer, but you MUST end with a line in this exact format:
        - If there are edges: Graph: A->B, B->C
        - If there are no edges: Graph: No edges
        
        Formatting rules:
        1) Use only the listed nodes. No self-loops. No cycles.
        2) The final answer line must start with "Graph:" (case-insensitive)
        3) If you provide reasoning, put it before the final "Graph:" line
        """
    ...
    def parse_llm_response(self, response: str) -> Optional[CausalGraph]:
        ...
        Supports both simple responses and responses with reasoning.
        """
        ...
        # Look for "Graph:" line (case-insensitive, multiline)
        # This will find the LAST occurrence of "Graph:" which should be the final answer
        matches = list(re.finditer(r'(?i)^.*\bgraph\s*:\s*(.+)$', s, flags=re.MULTILINE))
        
        if matches:
            # Use the last match (final answer)
            line = matches[-1].group(1).strip()
        else:
            # Fallback: try first line if no "Graph:" found
...
def setup_llm(llm_type: str, **kwargs) -> LLMInterface:
    ...
    elif llm_type in ("baidu", "qianfan"):
        api_key = kwargs.get('api_key') or os.environ.get('BAIDU_API_KEY') or os.environ.get('QIANFAN_API_KEY')
        if not api_key:
            raise ValueError("Baidu/Qianfan API key required")
        
        return BaiduQianfanLLM(
