...
from modules.llm_interface import LLMInterface, OpenRouterLLM, OpenAILLM, AnthropicLLM, LocalHFLLM
...
class Benchmark3D:
    ...
    def create_prompt(self, observations, prior_structures=None, compact: bool = False, history_limit: int = 2) -> str:
        """
        Build the prompt for the LLM.

        Args:
            observations: observation(s) in supported formats
            prior_structures: list[Structure3D] of prior unique structures to avoid repetition
            compact: if True, keep prompt concise for small local models
            history_limit: include up to last N prior structures
        """
        grid_size = None
        ...
        if isinstance(observations, str):
            ...
            if isinstance(observations[0], dict):
                obs_arr = np.array(observations[0]['observation'])
            else:
                obs_arr = self._parse_observation(observations[0]) if isinstance(observations[0], str) else np.array(observations[0])
            grid_size = obs_arr.shape[0]
        else:
            obs_arr = self._parse_observation(observations)
            grid_size = obs_arr.shape[0]
        ...
        prompt = f"""
        You are given observations of a 3D structure made of unit blocks on a {grid_size}x{grid_size} grid.
        ...
        # Prior structures (limited)
        if prior_structures and len(prior_structures) > 0:
            priors = prior_structures[-max(0, int(history_limit)):] if history_limit else []
            if priors:
                prompt += "\n\nPrior 3D structure generated (do not repeat if avoidable):\n"
                for idx, struct in enumerate(priors, 1):
                    prompt += f"\nAttempt {idx}:\n"
                    prompt += struct.to_string() + "\n"

        # Constraints
        if compact:
            # Minimal, strict output policy for small models
            prompt += f"""

        Task: Infer a valid 3D structure (grid {grid_size}x{grid_size}, up to {max_height} layers) that matches the top view(s).
        Constraints:
        - Layer 1 is the bottom layer and must not be all zeros.
        - Every block at layer z>1 must be supported by a block directly below (same (r,c)).
        - No unnecessary empty layers at bottom or top.
        - Total height <= {max_height}.

        Output format (strict, no extra text):
        Structure:
        Layer 1:
        [r1 values separated by spaces]
        [r2 values separated by spaces]
        ...
        Layer 2:
        ...
        (Continue only if needed)

        Start your reply exactly with "Structure:" and do not include any analysis or explanation.
        """
        else:
            # Full verbose guidance (original long version)
            prompt += f"""
    ...
    def evaluate_single_observation_set(
        ...
        for i in range(n_queries):
            # Use compact prompt and limited history for local models by default
            use_compact = False # isinstance(llm, LocalHFLLM)   # 先暂时不用，让prompt回归正常
            history_limit = 2 if use_compact else 4
            ...
            # If the LLM exposes token budget, shrink prompt further if needed
            try:
                if hasattr(llm, "count_tokens") and hasattr(llm, "max_context_tokens"):
                    # Reserve budget for completion
                    completion_budget = getattr(llm, "max_new_tokens", 256)
                    safety_margin = 64
                    max_input_tokens = max(512, llm.max_context_tokens - completion_budget - safety_margin)
                    ptoks = llm.count_tokens(prompt)
                    if ptoks > max_input_tokens:
                        # First fallback: compact + no history
                        prompt = self.create_prompt(
                            observations,
                            prior_structures=[],
                            compact=True,
                        ...
                        # Second fallback: ultra-minimal directive
                        if ptoks > max_input_tokens:
                            obs_arr = self._parse_observation(observations if isinstance(observations, str) else observation_set.get('observation', observations))
                            g = obs_arr.shape[0]
                            h = self.metadata.get('max_height', 3)
                            minimal = [
                                "Structure from top view.",
                                "Constraints: bottom layer non-empty; support required; no unnecessary empty layers; height limit.",
                                "Output only:",
                                "Structure:",
                                "Layer 1:",
                            ]
                            ...
                            # Insert the observation in minimal form
                            minimal_prompt += "\n" + "\n".join(" ".join(str(c) for c in row) for row in obs_arr) + "\n"
                            minimal_prompt += "Do not include any explanation."
                            prompt = minimal_prompt
            except Exception:
                # If anything goes wrong, keep the original prompt
                pass
        ...
        obs_id = observation_set.get('observation_id', observation_set.get('observations', [{}])[0].get('observation_set_id', 'unknown'))
    ...
    def run_benchmark(
        ...
        for idx in range(start_idx, len(sampled_sets)):
            ...
            if verbose:
                ...
                obs_id = obs_set.get('observation_id', obs_set.get('observations', [{}])[0].get('observation_set_id', 'unknown'))
